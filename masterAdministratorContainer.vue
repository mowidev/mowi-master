<template>
  <div> 
    ...
    <div v-for="(item, i) in filters" :key="`A-${i}`">
      ...
      <div  style="margin-top: 10px;" v-if="item.filterType == 'inputComponent' && item.selectField == true"> <inputComponent :showOp="item.operators"  :type="item.type" :isRequired="item.isRequired" :valueInput="item.valueInput"  :label="item.label" :validation="item.validation" :minimumCharacters="item.minimumCharacters" :nameInput="item.nameImput" :callbackData="item.callbackData"  :ref="item.name"
